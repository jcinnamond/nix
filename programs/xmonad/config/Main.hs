...
import XMonad.Layout.Conditional (ConditionalLayout, conditionalLayout)
...
import XMonad.Util.NamedScratchpad (NamedScratchpad (..), customFloating, defaultFloating, namedScratchpadAction)
...
myManageHook =
    composeAll
        ...
        , title =? "Picture-in-picture" --> doF copyToAll
        , className =? "quake" --> doRectFloat (W.RationalRect (1 / 4) (1 / 8) (1 / 2) (5 / 8))
        ]
...
scratchpads =
    [ NS "term" "kitty --class=quake" (className =? "quake") defaultFloating
    ]
...
wmKeys =
    ...
    , -- scratchpads
      ((xmod, xK_h), namedScratchpadAction scratchpads "term")
    , -- Workspaces
...
handlePopout :: (LayoutClass l Window) => l Window -> ConditionalLayout (PiP l) l Window
handlePopout l = conditionalLayout popoutProp (PiP popoutProp 1200 l) l

popoutProp :: Property
popoutProp = Title "Picture-in-picture" `Or` Role "pop-up"
